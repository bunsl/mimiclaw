...
MimiClawは小さなESP32-S3ボードをパーソナルAIアシスタントに変えます。USB電源に接続し、WiFiにつなげて、Feishuから話しかけるだけ — どんなタスクも処理し、ローカルメモリで時間とともに成長します — すべて親指サイズのチップ上で。

## MimiClawの特徴

- **超小型** — Linux不要、Node.js不要、無駄なし — 純粋なCのみ
- **便利** — Feishuでメッセージを送るだけ、あとはお任せ
...
Feishuでメッセージを送ると、ESP32-S3がWiFi経由で受信し、エージェントループに送ります — LLMが思考し、ツールを呼び出し、メモリを読み取り — 返答を送り返します。**Anthropic (Claude)**、**OpenAI (GPT)**、**MiniMax**、**Volcengine Ark** をサポートし、実行時に切り替え可能です。すべてが$5のチップ上で動作し、データはすべてローカルのFlashに保存されます。
...
- **USB Type-Cケーブル**
- **Feishu App ID と App Secret** — [open.feishu.cn](https://open.feishu.cn/) でアプリを作成して取得
- **Anthropic APIキー** — [console.anthropic.com](https://console.anthropic.com) から取得、**OpenAI APIキー** — [platform.openai.com](https://platform.openai.com) から取得、**MiniMax APIキー**、または **Volcengine Ark APIキー**
...
#define MIMI_SECRET_MODEL_PROVIDER  "anthropic"     // "anthropic"、"openai"、"minimax"、または "volcengine"
#define MIMI_SECRET_FEISHU_API_KEY          ""       // 任意: Feishu専用のAPIキー上書き
#define MIMI_SECRET_FEISHU_MODEL            ""       // 任意: Feishu専用のモデル上書き
#define MIMI_SECRET_FEISHU_MODEL_PROVIDER   ""       // 任意: Feishu専用のprovider上書き
#define MIMI_SECRET_WEBSOCKET_API_KEY       ""       // 任意: WebSocket専用のAPIキー上書き
#define MIMI_SECRET_WEBSOCKET_MODEL         ""       // 任意: WebSocket専用のモデル上書き
#define MIMI_SECRET_WEBSOCKET_MODEL_PROVIDER ""      // 任意: WebSocket専用のprovider上書き
#define MIMI_SECRET_CLI_API_KEY             ""       // 任意: CLI専用のAPIキー上書き
#define MIMI_SECRET_CLI_MODEL               ""       // 任意: CLI専用のモデル上書き
#define MIMI_SECRET_CLI_MODEL_PROVIDER      ""       // 任意: CLI専用のprovider上書き
#define MIMI_SECRET_SYSTEM_API_KEY          ""       // 任意: system/cron専用のAPIキー上書き
#define MIMI_SECRET_SYSTEM_MODEL            ""       // 任意: system/cron専用のモデル上書き
#define MIMI_SECRET_SYSTEM_MODEL_PROVIDER   ""       // 任意: system/cron専用のprovider上書き
...
mimi> set_api_key sk-ant-api03-... # APIキーを変更（Anthropic/OpenAI/MiniMax/Volcengine）
mimi> set_model_provider minimax   # プロバイダーを切替（anthropic|openai|minimax|volcengine）
mimi> set_channel_api_key feishu <key>      # 1つのチャネルだけAPIキーを上書き
mimi> set_channel_model feishu ep-2025...   # 1つのチャネルだけモデルを上書き
mimi> set_channel_provider feishu volcengine # 1つのチャネルだけproviderを上書き
mimi> set_channel_api_key feishu default     # チャネルAPIキー上書きを解除
mimi> set_channel_model feishu default       # チャネルモデル上書きを解除
mimi> set_channel_provider feishu default    # チャネルprovider上書きを解除
...
| `chat_12345.jsonl` | チャット履歴 — ボットとの会話 |

## ツール

MimiClawはAnthropic、OpenAI、MiniMax、Volcengine Arkのツール呼び出しをサポート — LLMは会話中にツールを呼び出し、タスクが完了するまでループします（ReActパターン）。
...
- **マルチプロバイダー** — Anthropic (Claude)、OpenAI (GPT)、MiniMax、Volcengine Ark をサポート、実行時に切り替え可能
- **Cronスケジューラ** — AIが定期・単発タスクを自律的にスケジュール、再起動後も永続化
- **ハートビート** — タスクファイルを定期チェックし、AIを自律的に駆動
- **ツール呼び出し** — ReActエージェントループ、4つのプロバイダーでツール呼び出し対応
