...
static void append_turn_context_prompt(char *prompt, size_t size, const mimi_msg_t *msg)
{
    ...
    int n = snprintf(
        ...
        "- Never use chat_id 'cron' when channel is a user-facing channel.\n",
...
static char *patch_tool_input_with_context(const llm_tool_call_t *call, const mimi_msg_t *msg)
{
    ...
    if (channel && strcmp(channel, MIMI_CHAN_SYSTEM) != 0 &&
        strcmp(channel, msg->channel) == 0 && msg->chat_id[0] != '\0') {
...
static void agent_loop_task(void *arg)
{
    ...
    while (1) {
        ...
        while (iteration < MIMI_AGENT_MAX_TOOL_ITER) {
            ...
            llm_response_t resp;
            err = llm_chat_tools_for_channel(msg.channel, system_prompt, messages, tools_json, &resp);
